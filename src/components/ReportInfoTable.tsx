...
export const ReportInfoTable: React.FC<ReportInfoTableProps> = ({
  ...
  return (
    <section className="mb-5 overflow-hidden rounded-md border border-slate-300">
      ...
      <table className="w-full text-right border-collapse text-xs sm:text-sm">
        <thead>
          <tr className="bg-emerald-50 text-emerald-950 font-bold border-b border-slate-300">
            <th className="p-2.5 sm:px-4 sm:py-3 w-1/3 border-l border-slate-300">بيانات التقرير</th>
            <th className="p-2.5 sm:px-4 sm:py-3 w-2/3">
              <div className="flex items-center justify-between">
                <span>التفاصيل</span>
                {isWeekly ? (
                  <span className="inline-block bg-emerald-800 text-white text-[11px] font-bold px-2 py-0.5 rounded">
                    التقرير الأسبوعي الشامل
                  </span>
                ) : (
                  <span className="inline-block bg-slate-800 text-emerald-300 text-[11px] font-bold px-2 py-0.5 rounded">
                    تقرير يومي معتمد
                  </span>
                )}
              </div>
        ...
        <tbody className="divide-y divide-slate-300 text-slate-800 bg-white">
          {/* Title Row */}
          <tr>
            <td className="p-2.5 sm:px-4 sm:py-2.5 font-bold bg-slate-50/70 border-l border-slate-300 text-slate-900">
              اسم التقرير / الموضوع:
            </td>
            <td className="p-2.5 sm:px-4 sm:py-2.5">
              {isReadOnly ? (
                ...
                <input
                  ...
                  className="w-full px-3 py-1.5 border border-slate-300 rounded focus:outline-hidden focus:ring-2 focus:ring-emerald-600 focus:border-emerald-600 font-medium text-slate-900 placeholder:text-slate-400"
                />
              )}
            </td>
          </tr>
                ...
                <td className="p-2.5 sm:px-4 sm:py-2.5">
                  {isReadOnly ? (
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="font-semibold text-slate-700">الأسبوع:</span>
                      <input
                        type="text"
                        value={weekNumber}
                        onChange={(e) => onWeekNumberChange && onWeekNumberChange(e.target.value)}
                        placeholder="الخامس / الأول ..."
                        className="w-32 px-2.5 py-1 border border-slate-300 rounded focus:ring-2 focus:ring-emerald-600 font-semibold"
                      />
                      <div className="no-print hidden sm:flex items-center gap-1 text-xs">
                  ...
                  {isReadOnly ? (
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="w-full max-w-lg px-3 py-1.5 border border-slate-300 rounded focus:ring-2 focus:ring-emerald-600 font-medium text-slate-900"
                      />
                    </div>
                  )}
                </td>
                ...
                <td className="p-2.5 sm:px-4 sm:py-2.5">
                  {isReadOnly ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="font-semibold text-slate-700 text-xs">اليوم:</span>
                      ...
                      <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-slate-100">
                        <span className="font-semibold text-slate-700 text-xs">التاريخ:</span>
                        <div className="flex items-center gap-1.5 font-medium">
                          <input
                            ...
                            placeholder="اليوم"
                            className="w-14 text-center px-2 py-1 border border-slate-300 rounded focus:ring-2 focus:ring-emerald-600 focus:border-emerald-600 font-semibold"
                          />
                          ...
                          <input
                            ...
                            placeholder="السنة"
                            className="w-20 text-center px-2 py-1 border border-slate-300 rounded focus:ring-2 focus:ring-emerald-600 focus:border-emerald-600 font-semibold"
                          />
                          <span className="font-bold text-slate-700">هـ</span>
                        </div>

                        <button
                          type="button"
                          onClick={setTodayDate}
                          className="no-print mr-auto inline-flex items-center gap-1 text-xs text-emerald-800 hover:text-emerald-950 font-semibold bg-emerald-50 hover:bg-emerald-100 px-2.5 py-1 rounded transition border border-emerald-200"
                ...
                <td className="p-2.5 sm:px-4 sm:py-2.5">
                  {isReadOnly ? (
                    ...
                    <div className="flex items-center gap-4">
                      <label className="inline-flex items-center gap-2 cursor-pointer text-slate-800 font-medium select-none">
                        <input
                          type="radio"
                          name="timeOfDay"
            ...
            <td className="p-2.5 sm:px-4 sm:py-2.5">
              {isReadOnly ? (
                ...
                <input
                  ...
                  className="w-full max-w-md px-3 py-1.5 border border-slate-300 rounded focus:outline-hidden focus:ring-2 focus:ring-emerald-600 font-semibold text-slate-900"
                />
              )}
            </td>
          </tr>
